          ...
          string winner, bool useDate, DateTime start, DateTime end, bool showMoves)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    char win = 'E';
                    string commandText = "SELECT (Events.Name) as eName, Site, Date, Result, Moves," +
                        "(select Name from Players where pID = WhitePlayer) as wName, (select Name from Players where pID = BlackPlayer) as bName, " +
                        "(select Elo from Players where pID = WhitePlayer) as wELo, (select Elo from Players where pID = BlackPlayer) as bElo " +
                        "FROM Events NATURAL JOIN Games WHERE true";
                    if (white != "")
                        commandText = commandText + " && @white = (SELECT Name FROM Players WHERE pID = WhitePlayer)";
                    if (black != "")
                        commandText = commandText + " && @black = (SELECT Name FROM Players WHERE pID = BlackPlayer)";
                    if (opening != "")
                        commandText = commandText + " && Moves LIKE @opening";
                    if (winner != "")
                    { 
                        commandText = commandText + " && Result = @winner";
                        win = winner[0];
                    }                        
                    if (useDate)
                        commandText = commandText + " && Date BETWEEN @start AND @end";

                    MySqlCommand readingCommand = conn.CreateCommand();
                    readingCommand.CommandText = commandText;
                    readingCommand.Parameters.AddWithValue("@white", white);
                    readingCommand.Parameters.AddWithValue("@black", black);
                    readingCommand.Parameters.AddWithValue("@opening", "1." + opening + "%");
                    readingCommand.Parameters.AddWithValue("@winner", win);
                    readingCommand.Parameters.AddWithValue("@start", start);
                    readingCommand.Parameters.AddWithValue("@end", end);

                    using(MySqlDataReader reader = readingCommand.ExecuteReader())
                    {
                        while(reader.Read())
                        {
                            string date = "";
                            ...
                            string game = "Event: " + reader["eName"] + "\r\nSite: " + reader["Site"] + "\r\nDate: " + date +
                              "\r\nWhite: " + reader["wName"] + " (" + reader["wElo"] + ")\r\nBlack: " + reader["bName"] + " (" + reader["bElo"] + ")\r\nResult: " + reader["Result"] + "\r\n";
                            if (showMoves)
                                game = game + "Moves: " + reader["Moves"] + "\r\n";
                            parsedResult = parsedResult + game + "\r\n";
                            numRows++;
                        }                       
                    }
