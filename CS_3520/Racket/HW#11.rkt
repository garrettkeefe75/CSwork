...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(restE a) (type-case Value (interp a env)
                 ...
                 [else (error 'interp "not a list")])]
    [(if0E tst thn els)
     (local [(define tst-result (interp tst env))
             (define thn-result (interp thn env))
             (define els-result (interp els env))]
       (cond
         [(equal? tst-result (numV 0)) thn-result]
         [else els-result]))]))
...
(define (typecheck [a : Exp] [tenv : Type-Env])
  (type-case Exp a
    ...
    [(consE l r)
     (local [(define left-type (typecheck l tenv))
             (define list-type (typecheck r tenv))]
       (begin
         (unify! list-type (listofT left-type) a)
         list-type))]
    [(firstE a)
     (local [(define result-type (varT (box (none))))
             (define list-type (typecheck a tenv))]
       (begin
    ...
    [(restE a)
     (local [(define result-type (listofT (varT (box (none)))))
       ...
       (begin
         ...
         result-type))]
    [(if0E tst thn els)
     (local [(define tstT (typecheck tst tenv))
             (define thnT (typecheck thn tenv))
...
(define (run-prog [s : S-Exp]) : S-Exp
  (local [(define parsed (parse s))
          (define type (typecheck parsed mt-env))
          (define result (interp parsed mt-env))]
