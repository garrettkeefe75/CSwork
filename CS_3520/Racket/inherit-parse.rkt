...
(define (parse [s : S-Exp]) : ExpI
  (cond
   ...
   [(s-exp-match? `{let {[SYMBOL ANY]} ANY} s)
    (local [(define inner (s-exp->list (first (s-exp->list (second (s-exp->list s))))))]
      (letI (s-exp->symbol (first inner))
            (parse (second inner))
            (parse (third (s-exp->list s)))))]
...
(module+ test
  ...
  (test (parse `{super m 1})
        (superI 'm (numI 1)))
  (test (parse `{let {[x 12]} x})
        (letI 'x (numI 12) (idI 'x)))
  (test (parse `{if0 1 {+ 1 2} {+ 0 3}})
        (if0I (numI 1) (plusI (numI 1) (numI 2)) (plusI (numI 0) (numI 3))))
  (test (parse `{newarray num 5 10})
        (newarrayI 'num (numI 5) (numI 10)))
  (test (parse `{arrayref {newarray num 5 10} 4})
        (arrayrefI (newarrayI 'num (numI 5) (numI 10)) (numI 4)))
  (test (parse `{arrayset {newarray num 5 10} 4 32})
        (arraysetI (newarrayI 'num (numI 5) (numI 10)) (numI 4) (numI 32)))
  (test/exn (parse `{nonsense})
            "invalid input")
...
(define (interp-prog [classes : (Listof S-Exp)] [a : S-Exp]) : S-Exp
  (let ([v (interp-i (parse a)
    ...
    (type-case Value v
      ...
      [(nulV) `object]
      [(arrayV type list) `array])));;might need to be diff
...
(module+ test
  ...
  (test (interp-prog
         (list
          `{class Empty extends Object
             {}})
         `{arrayset {newarray num 5 10} 4 32})
        `0)
 ...
 (test (interp-prog 
  ...
  (test (interp-prog 
        (list
         ...
         `{class PosnTest extends Posn
            {z}
            [mdist {arg} {super mdist arg}]})
        
        `{send {new PosnTest 5 3 null} addDist {new Posn 2 7}})
       `17)
  ...
  (test/exn (interp-prog 
        ...
        `{send {new Posn3D null 3 1} addDist {new Posn 2 7}})
       "")
