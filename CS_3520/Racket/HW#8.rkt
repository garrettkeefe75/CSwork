...
(module+ test
  ...
  (test (parse `{box 10}) (boxE (numE 10)))
  (test (parse `{unbox {box 20}}) (unboxE (boxE (numE 20))))
  (test (parse `{box? {box 30}}) (box?E (boxE (numE 30))))
...
(define (compile a-fae env)
  (type-case Exp a-fae
    ...
    [(boxE elem) (code-malloc1 18 (compile elem env))]
    [(unboxE box) (code-malloc1 19 (compile box env))]
    [(box?E elem) (code-malloc1 20 (compile elem env))]))
