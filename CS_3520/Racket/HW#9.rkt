      ...
      (type-case Exp a
        ...
        [(newE class-name field-exprs)
         (local [(define c (find classes class-name))
                 ...
                 (define super-name (classC-super-name c))]
           (if (= (length vals) (length (classC-field-names c)))
               (objV class-name vals super-name)
               (error 'interp "wrong field count")))]
        [(getE obj-expr field-name)
         (type-case Value (recur obj-expr)
           [(objV class-name field-vals super-name)
        ...
        [(ssendE obj-expr class-name method-name arg-expr)
         (local [(define obj (recur obj-expr))
           ...
           (call-method class-name method-name classes
                        obj arg-val))]
        [(selectE num obj)
         (type-case Value (interp num classes this-val arg-val)
          [(numV n)(cond
           [(equal? 0 n) (call-method (objV-class-name (interp obj classes this-val arg-val)) 'zero classes (interp obj classes this-val arg-val) arg-val)]
           [else (call-method (objV-class-name (interp obj classes this-val arg-val)) 'nonzero classes (interp obj classes this-val arg-val) arg-val)])]
          [else (error 'interp "not a number")])]
        [(instanceOfE obj name)
         (local [(define object (recur obj))]
           (cond
           [(equal? name 'Object)(numV 0)]
           [(equal? name (objV-class-name object))(numV 0)]
           [(equal? name (objV-super-name object))(numV 0)]
           [else (check-supers (objV-super-name object) name classes)]))]))))

(define (check-supers [super-name : Symbol][name : Symbol][classes : (Listof (Symbol * Class))])
  (local [(define class (find classes super-name))]
    (cond
      [(equal? name (classC-super-name class))(numV 0)]
      [(equal? 'Object (classC-super-name class))(numV 1)]
      [else (check-supers (classC-super-name class) name classes)])))
...
(module+ test
  (test (interp (numE 10) 
                empty (objV 'Object empty 'Object) (numV 0))
        (numV 10))
  ...
  (test (interp-posn (newE 'Posn (list (numE 2) (numE 7))))
        (objV 'Posn (list (numV 2) (numV 7)) 'Object))
  ...
  (test/exn (interp-posn (newE 'Posn (list (numE 0))))
            "wrong field count")
  (test/exn (interp-posn (selectE (newE 'Posn (list (numE 2) (numE 7))) (newE 'Posn (list (numE 2) (numE 7)))))
            "not a number"))
;------------------------inherit-------------------------------------------------------------------;
  ...
  (type-case Class (find classes-not-flat name)
    [(classC field-names methods super-name)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC super-field-names super-methods supers-super-name)
        (classC
  ...
  (local [(define a (exp-i->c i-a 'Object))
          ...
          (define classes
            (map (lambda (c)
                 ...
                 classes-not-flat))]
    (interp a classes (objV 'Object empty 'Object) (numV 0))))
  ...
  (cond
   ...
   [(s-exp-match? `{select ANY ANY} s)
    (selectI (parse (second (s-exp->list s)))
             (parse (third (s-exp->list s))))]
   [(s-exp-match? `{instanceof ANY ANY} s)
...
(define (interp-prog [classes : (Listof S-Exp)] [a : S-Exp]) : S-Exp
  (let ([v (interp-i (parse a)
                     (cons (values 'Object (classI 'Object empty empty))(map parse-class classes)))])
    (type-case Value v
      [(numV n) (number->s-exp n)]
      [(objV class-name field-vals super-name) `object])))
...
(module+ test
 ...
 (test (interp-prog 
  ...
  (test (interp-prog (list `{class Fish extends Object
                           ...
                           `{class Hammerhead extends Shark
                              {}}
                           `{class Spotted-Hammerhead extends Hammerhead
                              {}})
                     `{instanceof {new Spotted-Hammerhead 1 2 3} Fish})
        `0)
