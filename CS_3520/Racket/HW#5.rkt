...
(define times
  `{lambda {x} {lambda {y} {letrec {[f {lambda {n}
                 {if0 n
                      0
                      {+ {f {+ n -1}} x}}}]}
            {f y}}}})
  ...
  (cond
    ...
    [(s-exp-match? `{letrec {[SYMBOL ANY]} ANY} s)
     (let ([bs (s-exp->list (first
                             (s-exp->list (second
                                           (s-exp->list s)))))])
       (parse `{{lambda {,(first bs)} ,(third (s-exp->list s))}{,mk-rec-fun {lambda {,(first bs)} ,(second bs)}}}))]
...
(module+ test
  ...
  (test (interp (parse `{letrec {[f {lambda {n}
                 ...
                 mt-env)
          (numV -10))
  (test (interp (parse (list->s-exp (list (list->s-exp (list plus `2)) `3))) mt-env)
        (numV 5))
  (test (interp (parse (list->s-exp (list `+ `2 `3))) mt-env)
        (numV 5))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times `2)) `10))) mt-env)
        (numV 20))
