...
(module+ test
  ...
  (test (parse `{fst {pair 1 2}})
        (fstE (pairE (numE 1) (numE 2))))
  (test (parse `{snd {pair 1 2}})
        (sndE (pairE (numE 1) (numE 2))))
  (test/exn (parse `{{+ 1 2}})
            "invalid input"))
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(fstE pair) (type-case Value (interp pair env)
                   [(pairV fst snd) (force fst)]
                   [else (error 'interp "not a pair")])]
    [(sndE pair) (type-case Value (interp pair env)
                   [(pairV fst snd) (force snd)]
                   [else (error 'interp "not a pair")])]))
...
(module+ test
  ...
  (test/exn (interp (parse `{let {[bad {lambda {x} {+ x y}}]}
                    ...
                    mt-env)
            "free variable")
  (test/exn (interp (parse `{fst 2}) mt-env)
            "not a pair")
  (test/exn (interp (parse `{snd 2}) mt-env)
            "not a pair")
...
(module+ test
  ...
  (test (num-zero? (numV 1))
        #f)
  (test/exn (num-zero? (interp (parse `{pair 1 2}) mt-env))
            "not a number"))
