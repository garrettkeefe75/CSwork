...
public abstract class SorterTest
{
	...
	@Test
	public void sortDuplicates2()
	{
		Sorter<Integer> sTest = uutInt();
		List<Integer> listTest = listFromArgs(1, 1, 1, 2, 2, 2, 3, 4, 3, 4, 5, 5);
		List<Integer> listRef = listFromArgs(1, 1, 1, 2, 2, 2, 3, 3, 4, 4, 5, 5);
		sTest.sort(listTest);
		assertEquals(listRef, listTest);
	}
	...
	{
		Sorter<Integer> sTest = uutInt();
		List<Integer> random = SortUtils.listOfRandomInts(150);
		sTest.sort(random);
		assertTrue(SortUtils.isSorted(random));
	}
	...
	{
		Sorter<Integer> sTest = uutInt();
		List<Integer> random = SortUtils.listOfReversedSortedInts(150);
		sTest.sort(random);
		assertTrue(SortUtils.isSorted(random));
	}
